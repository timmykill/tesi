...
The proof of completeness of the deterministic semantics follows from the work from Cruz-Filipe, Montesi and Peressotti\cite{cruz2023formal}\footnote{The language and semantics defined in the paper has some trivial differences yet it is easy to see how the proofs presented there can be carried over to the language and semantics used in this thesis} which proves some important properties of the semantics, those being:
\begin{itemize}
...
\forall\,\Gamma \in \mathcal{G}.\ \Delta;\Gamma;pc \vdash C
$$
This property can be stated as follows: every declared procedure must typecheck under every security environment that \(\Delta\) admits for it (at any program counter). It is to be noted that we do the lookup yields an empty set of admissible environments in the case of untypable procedure at a certain $pc \in \MCL$ i.e.\ \(\Delta~X~pc=\varnothing\).
...
$$
\end{itemize}
Those are the two properties used for the previous proof of non-interference with respect to $\Delta$. In the following sections we will take ideas and terminology from the practice of \emph{type reconstruction}\cite{pierce2002types}.
...
We now define the procedure of bound reconstruction $\vdash e \rhd \uppsi$ that, given an \textbf{Expr} $e$ it returns the \emph{correct} bound:
\begin{mathpar}
...
\noindent\textbf{Proof:}\\
We proceed by induction on $\cdot \vdash \cdot \rhd \cdot$
\begin{itemize}
...
The main idea remains the same as before, with a few changes.
We firstly define the syntactic category \textbf{Bound}:
$$
\Uppsi ::= \bot \mid p.x \mid \eta \mid \Uppsi \sqcup \Uppsi
$$
where $p.x \in \textbf{Pid} \times \textbf{Var}$, $\eta$ for a new set \textbf{Fresh} of variables \emph{always considered sufficiently fresh}. $\eta$ will represent the security level of $pc$ in bounds.

We also define the syntactic category \textbf{Constraint} (the name is self-explanatory):
$$
\omega ::= \Uppsi \sqsubseteq p.x
$$
We also define a \emph{constraint context} (\textbf{ProcConstr}) $\delta: \textbf{ProcName} \rightarrow \textbf{Finset Constraint}$ which maps every procedure to the associated finite set of constraints.\\
...
$$
Substitution in constraint sets is defined naturally.\\
The function bind is also defined as \emph{lifting} an \textbf{ExprBound} into a \textbf{Bond} by binding every \textbf{Var} into the corresponding $\textbf{Pid} \times \textbf{Var}$
...
$$
Then the only element of \textbf{Fresh} that can appear in $E$ is $\eta$. It is to be noted that it is admitted for $E$ to have no element of \textbf{Fresh}.
...
\end{align*}
$\llbracket \cdot\rrbracket$ for \textbf{Bound} is the natural extension from the one for \textbf{ExprBound}.
\begin{align*}
\text{\underline{gen}}~&(\delta: \textbf{ProcConstr}): \textbf{ProcName} \rightarrow \MCL \rightarrow \textbf{Finset SecCLab} :=\\
&\lambda~(X: \textbf{ProcName}).~\lambda~(pc: \MCL).~\{ \Gamma \mid \text{\underline{cansolve}}~(\delta~X)~\Gamma~pc\}
\end{align*}
...
$$
Where $X(\vec{q}) = C_X \in \mathscr{C}$ and $E_X$ is computed with $\delta, \eta\vdash C_X \rhd E_X$\\
...
\item\emph{case: communication.} The case follows similarly to the previous one
\item\emph{case: conditional.} We have the following hypothesis:
\begin{align}
&\vdash e \rhd \upphi_e \tag{H1}\\
&\Uppsi_p =~\text{bind}~\uppsi_e~p\tag{H2}\\
...
\forall~~pc^\star,~&\text{\underline{cansolve}}~E_1~\Gamma~pc^\star \Rightarrow \Delta, \Gamma^\star, pc^\star \vdash C_1 \tag{IH1}\\
\forall~pc^\star,~&\text{\underline{cansolve}}~E_2~\Gamma~pc^\star \Rightarrow \Delta, \Gamma^\star, pc^\star \vdash C_2 \tag{IH2}
\end{align}
...
&\Rightarrow (\forall pc', \llbracket \Uppsi \rrbracket~\Gamma~pc'=\ell')\\
&\Rightarrow \text{\underline{cansolve}}~E~\Gamma~(pc\sqcup\ell')
\end{align*}
