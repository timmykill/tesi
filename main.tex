...
As we did for choreographies, we define a \emph{lifting} function $\lceil \cdot \rceil$ that maintains the structure of the term while changing it's type. \textbf{TODO: NO, LIFTING PER CSTORE SERVE DARGLI GAMMA PER COSTRUIRE [CSTORE] BEN TIPATO}
...
To be able to prove the theorem in \textbf{TODO: gestisci le ref a teoremi/assunzioni} we will firstly assume a few auxilliary lemmas, which are proven in appendix \textbf{???}
...
It is easy to see how the preservation lemma cam be easily carried over to $\cdot \twoheadrightarrow \cdot$.
...
\noindent\textbf{Proof:}\\
We start by \emph{lifting} $C,~\Sigma_1,~\Sigma_2$
