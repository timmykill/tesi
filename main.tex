...
\begin{equation}
\label{aux:compl_wr}
	\langle \wr C \wr, \Sigma, \mathscr{C} \rangle \Downarrow^{M^*} \Sigma'
\end{equation}
...
\begin{itemize}
	...
	\item \emph{size: n+1} In this case we have two hypothesis:
		\begin{align}
			...
			\exists M',~\langle \wr C'\wr, \Sigma', \mathscr{C} \rangle& \Downarrow^{M'} \Sigma'' \tag{IH} \label{aux:compl_wr_ih}
		\end{align}
		...
		\begin{itemize}
			...
			By definition of $\wr \cdot \wr$ we have: $\wr I; C' \wr = I;~\wr C' \wr$, thus using (\ref{aux:compl_wr_ih}) exists $M^* = \mu :: M'$
			\item \emph{selection, runtime term}:
				By further inversion of (\ref{aux:compl_wr_h1}) we find $C^\bigstar = C'$.\\
				By definition of $\wr \cdot \wr$ we have: $\wr I; C' \wr = \wr C'\wr$, thus using (\ref{aux:compl_wr_ih}) exists $M^* = M'$
			\item \emph{function call}: By further inversion of (\ref{aux:compl_wr_h1}) we find \textbf{TODO: TROVIAMO CHE SEI STUPIDO}
		\end{itemize}
	
\end{itemize}


\subsection{Extension of typing}\label{aux:ext_tj}
This lemma creates tells us \emph{lifting} a choreography maintains typing.\\
Formally:
$$
\Delta;\Gamma;\bot \vdash C \Rightarrow
