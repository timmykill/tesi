...
\begin{samepage}
\begin{verbatim}
def SecFunCtx := ProcName -> L -> (List Pid x Finset (@SecCLab L))
\end{verbatim}
\end{samepage}
...
inductive BrChor.lto {G: @SecCLab L}: BrConf -> BrConf -> Prop where
    ...
\end{verbatim}
\end{samepage}
...
\item \emph{procedure call}: We first rewrite the goal so that the call is exposed with the actual parameter renamings applied. We then unfold the freshness assumption \texttt{fresh\_functx} to obtain the two disjointness facts needed to commute the two independent renamings (on the formal parameters of $X$ and on the $\vec{q}$ of the outer substitution). With these in place we can apply the typing rule \texttt{tj\_bc.tcall}, discharging its \emph{declaration} and \emph{freshness} premises with \texttt{Hg} and \texttt{Hqs}. The remaining (non-trivial) obligation concerns the result-context side condition: we transport \texttt{Hgam} along the equality produced by \texttt{gamsubsts\_helper} (commutation under disjoint substitutions), using \texttt{gam\_res\_eq\_symm} and \texttt{gam\_res\_eq\_trans} to align orientations. Finally, the arity check is settled by \texttt{Hlen} together with \texttt{pids\_substs\_length}, which ensures that applying \texttt{pid\_substs} preserves list lengths. Altogether, these steps re-establish the call typing judgment under the renamed context, completing this case.
\end{itemize}
